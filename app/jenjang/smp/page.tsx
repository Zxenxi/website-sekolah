...
// Fungsi untuk mengambil data halaman SMP dari Strapi
async function getHalamanSMP(): Promise<HalamanJenjang | null> {
  const apiUrl = `${process.env.NEXT_PUBLIC_STRAPI_API_URL || 'http://localhost:1337'}/api/halaman-smp?populate=*`;
  
  try {
    const res = await fetch(apiUrl, { cache: 'no-store' });
    if (!res.ok) {
      throw new Error(`Gagal mengambil data Halaman SMP: ${res.statusText}`);
    }
    
    const body = await res.json();
    
    if (!body.data) {
        console.error("Data Halaman SMP tidak ditemukan. Pastikan sudah di-publish.");
        return null;
    }
    ...
    console.error("Terjadi error di fungsi getHalamanSMP:", error);
    return null;
  }
}

// Komponen Halaman Jenjang SMP
export default async function HalamanSMPPage() {
  const dataHalaman = await getHalamanSMP();
